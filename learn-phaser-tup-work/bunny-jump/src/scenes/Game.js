...
export default class Game extends Phaser.Scene {
    ...
    create() {
        ...
        const style = { color: "#FF5F1F", fontSize: 24 }
        this.carrotsCollectedText = this.add.text(240, 10, "Carrots: 0", style)
            .setScrollFactor(0)
            .setOrigin(0.5, 0)

        const randomEventDelay = Phaser.Math.Between(1000, 30000)
        this.time.addEvent({
    ...
    update() {
        ...
        this.platforms.children.iterate(child => {
            ...
            if (platform.y >= scrollY + 700) {
                platform.y = scrollY - Phaser.Math.Between(85, 100)
                platform.body.updateFromGameObject()

                this.addCarrotAbove(platform)

                if (carrot.y >= this.cameras.main.scrollY + 700) {
                    carrot.y = scrollY - Phaser.Math.Between(50, 90)
        ...
        if (vy > -180 && this.player.texture.key !== "bunny-stand") {
